...
#             for step, (inputs, labels) in enumerate(dataloader):
#                 # 打印批次大小用于调试
#                 # logger.info(f"Batch size: {inputs.size(0)}")
#                 if step % 1000 == 0:
#                     logger.info(f"Epoch {epoch}, Phase: {phase}, Step: {step}/{len(dataloader)}")
#                 inputs = inputs.to(device)
#                 labels = labels.to(device)
...
#                 running_loss += loss.item() * inputs.size(0)
#                 if step % 1000 == 0:
#                     logger.info(f"Step {step} with loss {loss.item():.4f}, Lr: {scheduler.get_last_lr()}")
#                 valence_preds.extend(outputs[:, 0].detach().cpu().numpy())
#                 valence_labels.extend(labels[:, 0].cpu().numpy())
#                 arousal_preds.extend(outputs[:, 1].detach().cpu().numpy())
#                 arousal_labels.extend(labels[:, 1].cpu().numpy())
...
if __name__ == "__main__":
    ...
    data_transforms = {
        ...
        'val': transforms.Compose([
            transforms.Resize(size=(256, 256), interpolation=Image.BICUBIC, antialias=True), # 将图像较短的边缩放到256, 保持图像的纵横比
    ...
    # 数据集路径
    train_img_dir = "/data/lyh/Affwild2/cropped_aligned/train"
    val_img_dir = "/data/lyh/Affwild2/cropped_aligned/val"
    train_label_dir = "/data/lyh/Affwild2/6th_ABAW_Annotations/train"
    val_label_dir = "/data/lyh/Affwild2/6th_ABAW_Annotations/val"
    ...
    # image_datasets = {
    #     'train': VADataset(train_img_dir, train_label_dir, transform=data_transforms['train']),
    #     'val': VADataset(val_img_dir, val_label_dir, transform=data_transforms['val'])
    # }
    # dataloaders = {
    ...
    optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-4) #如果还过拟合设置更大的weight_decay=1e-3, 但是验证集上表现不好,可以减少
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5, min_lr=1e-6, verbose=True)
    # 训练模型
    model = train_model(model, train_dataset, val_dataloader, criterion, optimizer, scheduler, num_epochs=100, patience=10, device=device)
